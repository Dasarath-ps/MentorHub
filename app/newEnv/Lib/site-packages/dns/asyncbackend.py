...
def get_backend(name: str) -> Backend:
    """Get the specified asynchronous app.

    *name*, a ``str``, the name of the app.  Currently the "trio"
    and "asyncio" backends are available.

    Raises NotImplementedError if an unknown app name is specified.
    """
    ...
    if name == "trio":
        ...
        raise NotImplementedError(f"unimplemented async app {name}")
...
def get_default_backend() -> Backend:
    """Get the default app, initializing it if necessary."""
    if _default_backend:
...
def set_default_backend(name: str) -> Backend:
    ...
    ``get_default_backend()`` will initialize the app
    appropriately in many cases.  If ``sniffio`` is not installed, or
    in testing situations, this function allows the app to be set
    explicitly.
    """
